...
export function CertificateList({
  ...
  const filtered = React.useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (needle) {
        const hay = `${row.serial_number} ${row.principal}`.toLowerCase();
  ...
  const columns: Array<DataTableColumn<Certificate>> = [
    {
      ...
      cell: (row) => (
        <Link
          href={`/admin/signer/certificates/${row.serial_number}`}
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Review and revoke SSH certificates issued by the certificate signer.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-md border bg-card p-4 sm:flex-row sm:items-center">
        <Input
          type="search"
          placeholder="Search by serial or principal"
          value={search}
        ...
        <span className="text-xs text-muted-foreground sm:ml-auto">
          Search and status filters apply to this page.
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          heading={
            (pagination?.total ?? rows.length) === 0
              ? "No certificates yet"
              : "No matches on this page"
          }
          description={
            (pagination?.total ?? rows.length) === 0
              ? "The signer has not issued any certificates."
              : "No certificates on the loaded page match the current filters."
          }
        />
      ) : (
        <DataTable
