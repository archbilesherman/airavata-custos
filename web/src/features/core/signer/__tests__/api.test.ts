...
describe("listCertificates", () => {
  it("round-trips pagination and returns the envelope", async () => {
    ...
    expect(url).toContain("/api/v1/signer/admin/certificates");
...
describe("getCertificate", () => {
  it("validates the response", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toContain("/api/v1/signer/admin/certificates/42");
  });
});
...
describe("revokeCertificate", () => {
  it("POSTs the reason to /certificates/{serial}/revoke and parses the response", async () => {
    ...
    expect(url).toContain("/api/v1/signer/admin/certificates/42/revoke");
  ...
  it("rejects when the backend denies the revoke (403)", async () => {
    fetchMock.mockResolvedValueOnce(
      mockResponse(403, {
        error: "forbidden",
        message: "Requires the signer:certificates:write privilege",
      }),
    );
